<template>
    <div class="container-fluid pl-0 pr-0 ml-0 mr-0" v-if="$gate.isEditor()">
                ...
                <div class="card pl-0 pr-0 ml-0 mr-0">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            <button class="btn btn-sm btn-dark" @click="writerSort(5)" v-if="$gate.isEditor()">Completed
                            </button>
                            <button class="btn btn-sm btn-danger" @click="writerSort(4)" v-if="$gate.isEditor()">Revision
                            </button>
                            <button class="btn btn-sm btn-info" @click="writerSort(1)" v-if="$gate.isEditor()">Assigned
                            </button>
                            <button class="btn btn-sm btn-success" @click="writerSort(3)" v-if="$gate.isEditor()">Uploaded
                            </button>
                        </div>
                    </div>
...
<script>
    export default {
        ...
        methods:{
            getResults(page = 1) {
                axios.get('api/myorderse?page=' + page)
                    .then(response => {
            ...
            writerSort(sort){
                if (this.$gate.isWriter()){
                    axios.get("/api/writersort/" + sort).then(({data}) => ([this.orders = data]));
                }
            },
            getMyOrders(){
                axios.get("/api/myorderse").then(({data}) => ([this.orders = data]));
            },
        },
        created() {
            Fire.$on('searching', ()=>{
                let query = this.$parent.search;
                axios.get('api/findMyOrder?q=' + query)
                    .then((data)=>{
